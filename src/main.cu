...
vector<long double> computeForceDensityStates(int ndim, double n1, double n2, double horizon, matrix& StiffnessTensor, double dx, vector<Particle*>& piNeighbors, Particle& pi, Particle& pj, const unordered_map<int, int>& globalLocalIDmap, vector<vector<double>>& bondDamage){
    ...
    if (pi.globalID == 0 && pj.globalID == 1) {
        cout << "CPU DEBUG: Tmatrix=[" << scientific << setprecision(6) << Tmatrix.elements[0][0] << "," << Tmatrix.elements[0][1] << "," << Tmatrix.elements[1][0] << "," << Tmatrix.elements[1][1] << "]" << endl;
        cout << "CPU DEBUG: bondIJ=[" << scientific << setprecision(6) << bondIJ[0] << "," << bondIJ[1] << "], horizonVolume=" << scientific << setprecision(6) << horizonVolume << endl;
    }
    ...
    if (pi.globalID == 0 && pj.globalID == 1) {
        cout << "CPU DEBUG: final_Tvector=[" << scientific << setprecision(6) << Tvector[0] << "," << Tvector[1] << "]" << endl;
    }
...
int main(int argc, char *argv[]){
    ...
    for (int j = 0; j < totalSteps; ++j){
        ...
        cout << "=== VELOCITY COMPUTATION TIMING RESULTS ===" << endl;
        cout << "GPU Time: " << gpu_duration.count() << " microseconds (" << gpu_duration.count() / 1000.0 << " ms)" << endl;
        cout << "============================================" << endl;
